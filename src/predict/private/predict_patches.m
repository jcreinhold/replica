...
%       fg: foreground indices (for saving result image)
    ...
    if isfield(ss, 't1w') && ~strcmp(ps.target, 't1w')
        [subject_images{end+1}, dim] = open_atlas(ss.t1w, ps.w4, ps.r4, ...
    ...
    if isfield(ss, 't2w') && ~strcmp(ps.target, 't2w')
        [subject_images{end+1}, ~] = open_atlas(ss.t2w, ps.w4, ps.r4, ...
                                         'WMPeakNormalize', ps.wm_peak_normalize, ...
                                         'isT1', false);
    end
    if isfield(ss, 'pdw') && ~strcmp(ps.target, 'pdw')
        [subject_images{end+1}, ~] = open_atlas(ss.pdw, ps.w4, ps.r4, ...
                                         'WMPeakNormalize', ps.wm_peak_normalize, ...
                                         'isT1', false);
    end
    if isfield(ss, 'flair') && ~strcmp(ps.target, 'flair')
        [subject_images{end+1}, ~] = open_atlas(ss.flair, ps.w4, ps.r4, ...
                                         'WMPeakNormalize', ps.wm_peak_normalize, ...
                                         'isT1', false);
    end
